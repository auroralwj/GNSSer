//2015.01.27, Cui Yang, created， geoid models
...
namespace Gnsser
{
    ...
    public class Geoid
    {
        ...
        string fp_geoid = null;         /* geoid file pointer */
        int model_geoid = GnssConst.GEOID_EMBEDDED; /* geoid model */


        /* bilinear interpolation ----------------------------------------------------*/
        public static double interpb(double[] y, double a, double b)
        {
        ...
        public double geoidh_emb(double[] pos)
        {
        ...
        #region EGM96 EGM2008 GIS等模型需要读取文件，然后计算，暂时没有加入
        ...
        //        trace(2,"geoid satData file range error: off=%ld\n",off);
        //    }
        ...
        //    /* notes: 4byte-zeros are inserted at prevObj and last field of a record */
        //    /*        for current geid satData files */
        ...
        //    return interpb(y,a,b);
        //}
        ///* get gsi geoid satData --------------------------------------------------------*/
        //static double fgetgsi(FILE *fp, int nlon, int nlat, int time, int j)
        //{
        //    const int nf=28,wf=9,nl=nf*wf+2,nr=(nlon-1)/nf+1;
        //    double v;
        //    long off=nl+j*nr*nl+time/nf*nl+time%nf*wf;
        //    char buff[16]="";

        //    if (fseek(fp,off,SEEK_SET)==EOF||fread(buff,wf,1,fp)<1) {
        //        trace(2,"out of range for gsi geoid: time=%d j=%d\n",time,j);
        //        return 0.0;
        //    }
        //    if (sscanf(buff,"%lf",&v)<1) {
        //        trace(2,"gsi geoid satData format error: time=%d j=%d buff=%s\n",time,j,buff);
        //        return 0.0;
        //    }
        //    return v;
        //}
        ...
        //        trace(2,"geoidh_gsi: satData outage (lat=%.3f lon=%.3f)\n",pos[0],pos[1]);
        //        return 0.0;
        //    }
        //    return interpb(y,a,b);
        //}
...
*-----------------------------------------------------------------------------*/
        public double geoidh(GeoCoord pos)
        {
            double[] posd = new double[2];
            double h;

            posd[1] = pos.Lon * SunMoonPosition.DegToRad;
            posd[0] = pos.Lat * SunMoonPosition.DegToRad;
            ...
            switch (model_geoid)
            {
                case GnssConst.GEOID_EMBEDDED: h = geoidh_emb(posd); break;
        ...
        private static double[] range = new double[4] { 0.00, 360.00, -90.00, 90.00 };      // new double[4];       /* embedded geoid area range {W,E,S,N} (deg) */


        private static float[][] geoid = {               // new float[361][]; /* embedded geoid heights (m) (lon x lat) */
new float[]{

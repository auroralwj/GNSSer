//2016.11.18,double, create in hongqing, 采用了GF方法[1,-1,0]三频探测周跳
...
namespace Gnsser
{
    ...
    /// 如果返回 1 表示有周跳。
    /// </summary>
    public class TriFreqGF1BaseOnPhaseCycleSlipDetector : TimeValueCycleSlipDetector<SmoothTimeValue>, ICycleSlipDetector
    {
        ...
        public TriFreqGF1BaseOnPhaseCycleSlipDetector(GnssProcessOption Option)
            : base(Option){ 
        }
        /// <summary>
        /// 周跳探测,并进行标记，而不修复。 默认构造函数。
        /// </summary>
        public TriFreqGF1BaseOnPhaseCycleSlipDetector(double maxBreakingEpochCount = 4, bool isUsingRecoredCsInfo=true)
            : base(maxBreakingEpochCount, isUsingRecoredCsInfo)
        {     
            log.Info("采用了GF方法[1,-1,0]三频探测周跳。");
        } 
        /// <summary>
        /// 周跳探测类型。
        /// </summary>
        ...
        {
            if(EpochSat.FrequencyCount <= 3)
            {
                throw new Exception("只支持三频数据！");
            }
            ...
            bool isCS = GetDetection(EpochSat.RecevingTime, EpochSat.Prn,
                EpochSat.Combinations.TriFreqBasedOnGF1Combination, lambdaLimit);
        ...
        //public TableObjectManager TableObjectManager { get; set; }
    }
}

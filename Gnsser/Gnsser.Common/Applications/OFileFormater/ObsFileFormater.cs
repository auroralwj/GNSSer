...
namespace Gnsser
{
    ...
    public class ObsFileFormater : ObsFileEpochRunner<RinexEpochObservation>
    {
        ...
        private RinexObsFileHeader BuildOutputHeader(RinexObsFileHeader OldHeader)
        {
            ...
            {
                newHeader.SiteInfo.SiteName = Geo.Utils.StringUtil.SubString(newHeader.FileName.ToUpper(), 0, Option.SiteNameLength.Value);
            }
            if (Option.SiteNameLength.Enabled)
            {
                newHeader.SiteInfo.SiteName = Geo.Utils.StringUtil.SubString(newHeader.SiteInfo.SiteName, 0, Option.SiteNameLength.Value);
            }
            ...
            if (Option.IsEnableRemoveIndicatedFrequence)
            {
                ...
                log.Info("将移除频率：" + Geo.Utils.StringUtil.ToString(Option.FrequenceNumToBeRemoved));
            }
        ...
        private void InitWriter(string outFilePath)
        {
            ...
            if (!IsOverrite && File.Exists(savePath))
            {
                ...
                savePath = Geo.Utils.PathUtil.GetAvailableName(savePath);

...
namespace Gnsser.Service
{
    ...
    /// </summary>
    public class AmbiFixedEpochDoublePhaseOnlytDifferPositioner : MultiSiteEpochSolver
    {
        #region 构造函数 
        /// <summary>
        /// 构造函数
        ...
        public AmbiFixedEpochDoublePhaseOnlytDifferPositioner(DataSourceContext Context, GnssProcessOption Option)
        :base( Context,   Option)
        { 
            this.ResidualsAnalysis = new ResidualsAnalysis();
            this.DualBandCycleSlipDetector = new DualBandCycleSlipDetector();
            this.Name = "简易单历元载波相位双差";
            ...
            this.MatrixBuilder = new AmbiFixedEpochDoublePhaseOnlytDifferMatrixBuilder(base.Option, BaseParamCount);
        }
        ...
        {
            var result = base.CaculateKalmanFilter(epochInfos, lastResult) as AmbiFixedEpochDoublePhaseOnlytDifferResult;
            //这里其实不用了，顶层已经做了
        ...
        {
            var result = new AmbiFixedEpochDoublePhaseOnlytDifferResult(this.CurrentMaterial, Adjustment, this.MatrixBuilder.GnssParamNameBuilder, CurrentBasePrn);
            return result;
        }
    }
}

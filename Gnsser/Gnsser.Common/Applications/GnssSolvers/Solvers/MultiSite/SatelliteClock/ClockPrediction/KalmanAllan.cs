...
namespace Gnsser
{
    ...
    /// 使用Allan方差的Kalman
    /// </summary>
    public class KalmanAllan : BasicFunctionModel
    {
        /// <summary>
        /// 计算
        /// </summary>
        ...
        public void Calculate(ArrayMatrix ModelData, ArrayMatrix CompareData, int IntervalSeconds)
        {
            ...
            //进行Kalman滤波解算
            ArrayMatrix X = GetKalmanFilter(ModelLength, InitialX, CovarianceV, InitialCovarianceX, InitialCovarianceW, ModelData, CovarianceByAllanDerivation);

            ArrayMatrix PredictedData = new ArrayMatrix(PredictedLength, 1);//预报数据
            ArrayMatrix TheNexRealData = new ArrayMatrix(PredictedLength, 1);//预报数据所对应的真实数据
            double x0 = X[0, 0]; double x1 = X[1, 0]; double x2 = X[2, 0];
            PredictedData = GetPredictedData(x0, x1, x2, IntervalSeconds, PredictedLength);
            //TheNexRealData = GetPredictedRealData(CompareData, PredictedLength);

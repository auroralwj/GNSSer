...
namespace Gnsser.Service
{
    ...
    /// 单频半和模型
    /// </summary>
    public class HalfSumSingeFreqMatrixBuilder : BasePppPositionMatrixBuilder
    {
        /// <summary>
        /// 单频半和模型构造函数。
        /// </summary> 
        /// <param name="option">解算选项</param> 
        public HalfSumSingeFreqMatrixBuilder(
             GnssProcessOption option)
            : base(option)
        {
            ParamNameBuilder = new HalfSumSingeFreqNameBuilder(option);//option中包含了几个系统    
        ...
        public virtual IVector ObservationVector
        {
            get
            {
                ...
                foreach (var item in  this.EnabledPrns)
                {
                    ...
                    int phaseIndex = satIndex + EnabledSatCount;
                    L[phaseIndex] = 0.5 * (rangeA + phaseRangeA);
                    L.ParamNames[phaseIndex] = item + "_LC";
        ...
        public override Matrix Coefficient
        {
            get
            {
                ...
                int satCount = CurrentMaterial.EnabledSatCount; 
                double[][] A = Geo.Utils.MatrixUtil.Create(this.ObsCount, this.ParamCount); 
                int rowOfRange = 0;
                ...
                {               
                    ...
                    //最后设置载波相位系数
                    int colOfAmbi = colIndex  + satIndex;

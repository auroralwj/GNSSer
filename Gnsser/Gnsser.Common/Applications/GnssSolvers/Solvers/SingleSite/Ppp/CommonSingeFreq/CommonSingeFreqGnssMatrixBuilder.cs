...
namespace Gnsser.Service
{
    ...
    /// 通用单站单频计算，高度可配置。
    /// </summary>
    public class CommonSingeFreqGnssMatrixBuilder : BasePppPositionMatrixBuilder
    {
        /// <summary>
        /// 通用单站单频计算，高度可配置 构造函数。
        /// </summary> 
        /// <param name="option">解算选项</param> 
        public CommonSingeFreqGnssMatrixBuilder(
             GnssProcessOption option)
            : base(option)
        {           
            ParamNameBuilder = new CommonSingeFreqGnssNameBuilder(option);//option中包含了几个系统    
        ...
        public override Matrix Coefficient
        {
            get
            {
                ...
                {               
                    ...
                    A[rowOfRange][colOfIono] =1;//电离层码延迟
                    A[rowOfPhase][colOfIono] =-1;//电离层波超前

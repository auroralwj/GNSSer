...
namespace Gnsser.Service
{
    ...
    /// </summary>
    public class TropAugIonoFreePpp : SingleSiteGnssSolver
    {
        #region 构造函数
        ...
        public TropAugIonoFreePpp(DataSourceContext DataSourceContext, GnssProcessOption PositionOption)
            : base(DataSourceContext, PositionOption)
        {
            ...
            else { this.MatrixBuilder = new TropAugIonoFreePppMatrixBuilder(PositionOption); }
        ...
        public override SingleSiteGnssResult CaculateKalmanFilter(EpochInformation epochInfo, SingleSiteGnssResult lastResult)
        {
            ...
            #region
            if (this.IsFixingAmbiguity && epochInfo.ReceiverTime.SecondsOfDay > epochInfo.ObsInfo.StartTime.SecondsOfDay + 120)//&& this.Adjustment.ParamCount>10000000
            {
                #region 读取自定义FCB文件
                string WLpath = Path.Combine(Setting.DataDirectory, @"GNSS\Attachment\2013001FCB\WLFCB.txt");//  "D:\\Data\\GNSS\\Rinex\\PPPAR\\19240FCB\\2017001\\WLFCB.txt";
                log.Error("？？？请采用路径参数替代宽巷绝对路径，" + WLpath);
                ...
                using (var streamReader = new StreamReader(WLpath))
                {
                    ...
                    while ((line = streamReader.ReadLine()) != null)
                    {
                        ...
                        var val = Double.Parse(line.Substring(4, 6));
                        SDWL_FCB.Add(prn, val);
                    }
                }
                string NLpath = Path.Combine(Setting.DataDirectory, @"GNSS\Attachment\2013001FCB\NLFCB.txt");// "D:\\Data\\GNSS\\Rinex\\PPPAR\\19240FCB\\2017001\\NLFCB.txt";
                log.Error("？？？请采用路径参数替代窄巷绝对路径，" + NLpath);
                using (var streamReader = new StreamReader(NLpath))
                {
                    ...
                    {
                        SatelliteNumber prn = SatelliteNumber.Parse(line.Substring(0, 3));
                        var val = Double.Parse(line.Substring(4, 7));
                ...
                if (epochInfo.ReceiverTime.SecondsOfDay == 57360)
                {
                    int a = 2;
                }
                ...
                for (int i = 4; i < this.Adjustment.ParamCount; i++) //没有对流层延迟，应从4开始
                {
                ...
                if (SDNL_floatAmb.Count >= 4) //模糊度维度>4的情况才进行固定
                {
                    ...
                    IMatrix B = new ArrayMatrix(this.Adjustment.ParamNames.Count - 5, this.Adjustment.ParamNames.Count - 4, 0.0); //将非差模糊度转换为星间单差模糊度的旋转矩阵
                    int index = 0;//参考星的索引位置
                    for (int i = 4; i < this.Adjustment.ParamNames.Count; i++)//没有对流层延迟，应从4开始
                    {
                        SatelliteNumber sat = SatelliteNumber.Parse(this.Adjustment.ParamNames[i].Substring(0, 3));
                        if (sat == MaxPrn)
                        {
                    ...
                    IMatrix covaAmb = new ArrayMatrix(this.Adjustment.ParamNames.Count - 4, this.Adjustment.ParamNames.Count - 4, 0); //非差模糊度的协方差矩阵
                    for (int i = 0; i < this.Adjustment.ParamNames.Count - 4; i++)
                    {
                        for (int j = 0; j < this.Adjustment.ParamNames.Count - 4; j++)
                        {
                            covaAmb[i, j] = this.Adjustment.CovaOfEstimatedParam[i + 4, j + 4];
                        }
                    }
                    IMatrix SD_covaAmb = B.Multiply(covaAmb).Multiply(BT); //星间单差模糊度的协方差矩阵
                    IMatrix SDNL_covaAmb = new ArrayMatrix(this.Adjustment.ParamNames.Count - 5, this.Adjustment.ParamNames.Count - 5, 0.0); //星间单差窄巷模糊度的协方差矩阵
                    for (int i = 0; i < this.Adjustment.ParamNames.Count - 5; i++)
                    {
                    ...
                    if (ratio > 3)
                    {
                        if (N1.Length == newtrix.RowCount * 2)//没有出现降维
                        {
                            ...
                            for (int i = 0; i < newtrix.RowCount; i++)
                            {
                                ...
                                double currentIonoAmb = aa * currentWL + bb * currentNL + bb * SDNL_FCB[sat];
                        ...
                        foreach (var sat in fixedIonoAmb.Keys)
                        {
                            ...
                            restrictMatrix[ii, 4 + index] = -1;
                            restrictMatrix[ii, 4 + currentIndex] = 1;
                        ...
                        {
                            P2[kk, kk] = 5 * Math.Pow(10, 6); //给FCB的一个权值
                        }

...
namespace Gnsser.Orbits
{
	...
	abstract internal class NoradBase
   {
      ...
      public abstract TimedMotionState GetPosition(double tsince);
      ...
      private void Initialize()
      {
         ...
         m_perigee = OrbitConsts.RadiusOfEquator * (m_aodp * (1.0 - m_satEcc) - OrbitConsts.Ae);

         m_s4      = OrbitConsts.S;
         m_qoms24  = OrbitConsts.Qoms2t;
         ...
         if (m_perigee < 156.0)
         {
            ...
            m_qoms24 = Math.Pow((120.0 - m_s4) * OrbitConsts.Ae / OrbitConsts.RadiusOfEquator, 4.0);
            m_s4 = m_s4 / OrbitConsts.RadiusOfEquator + OrbitConsts.Ae;
         }
         ...
         double c2 = m_coef1 * m_xnodp * 
                     ...
                     0.75 * OrbitConsts.Ck2 * m_tsi / psisq * m_x3thm1 * 
                     (8.0 + 3.0 * m_etasq * (8.0 + m_etasq)));

         m_c1    = Orbit.BStar * c2;
         m_sinio = Math.Sin(m_satInc);

         double a3ovk2 = -OrbitConsts.Xj3 / OrbitConsts.Ck2 * Math.Pow(OrbitConsts.Ae,3.0);

         m_c3     = m_coef * m_tsi * a3ovk2 * m_xnodp * OrbitConsts.Ae * m_sinio / m_satEcc;
         m_x1mth2 = 1.0 - m_theta2;
         m_c4     = 2.0 * m_xnodp * m_coef1 * m_aodp * m_betao2 * 
                    (m_eta * (2.0 + 0.5 * m_etasq) +
                    m_satEcc * (0.5 + 2.0 * m_etasq) - 
                    2.0 * OrbitConsts.Ck2 * m_tsi / (m_aodp * psisq) *
         ...
         double temp1  = 3.0 * OrbitConsts.Ck2 * pinvsq * m_xnodp;
         double temp2  = temp1 * OrbitConsts.Ck2 * pinvsq;
         double temp3  = 1.25 * OrbitConsts.Ck4 * pinvsq * pinvsq * m_xnodp;
      ...
      /// <returns></returns>
      protected TimedMotionState FinalPosition(double  incl, double omega,  double     e,   
                                      ...
                                      double    xn, double tsince)
      {
         ...
         double capu = GeoMath.ModTwoPI(xlt - xnode);
         double temp2  = capu;
         double temp3  = 0.0;
         double temp4  = 0.0;
         double temp5  = 0.0;
         ...
         XYZ vecPos = new XYZ(x, y, z);
         DateTime gmt = Orbit.EpochTime.AddMinutes(tsince);

         // Validate on altitude
         double altKm = (vecPos.Magnitude() * (OrbitConsts.RadiusOfEquator / OrbitConsts.Ae));

         if (altKm < OrbitConsts.RadiusOfEquator)
         {
            throw new Exception("高度错误，" + gmt + Orbit.SatNameLong);
         }
         ...
         XYZ vecVel = new XYZ(xdot, ydot, zdot);

         return new TimedMotionState(vecPos, vecVel, new Julian(gmt));
      }
   }
}

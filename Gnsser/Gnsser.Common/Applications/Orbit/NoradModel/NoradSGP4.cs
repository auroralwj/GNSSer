...
namespace Gnsser.Orbits
{
	...
	internal class NoradSGP4 : NoradBase
	{
         ...
         base(orbit)
      {
         ...
         m_xmcof  = -(2.0 / 3.0) * m_coef * Orbit.BStar * OrbitConsts.Ae / m_eeta;
      ...
      public override TimedMotionState GetPosition(double tsince)
      {
         ...
         if ((m_aodp * (1.0 - m_satEcc) / OrbitConsts.Ae) < (220.0 / OrbitConsts.RadiusOfEquator + OrbitConsts.Ae))
         {
            isimp = true;
         }
         ...
         double a = m_aodp * GeoMath.Sqr(tempa);
         double e  = m_satEcc - tempe;

         double xl = xmp + omega + xnode + m_xnodp * templ;
         double xn = OrbitConsts.Xke / Math.Pow(a, 1.5);

...
namespace Gnsser.Orbits
{
	...
	internal class NoradSDP4 : NoradBase
	{
         ...
         base(orbit)
      {
         ...
         dp_thgr = jd.GetGreenwichMeanSiderealTime();
         ...
         double eosq = GeoMath.Sqr(Orbit.Eccentricity);

         // Apply the solar and lunar terms on the prevObj pass, then re-apply the
         ...
         // Mean motion is given in radians per second.
                               ...
                               ref double tsince)
      {
         ...
         {
            xincc  = -xincc;
            xnode  = xnode  + OrbitConsts.PI;
            omgadf = omgadf - OrbitConsts.PI;
         }
                                 ...
                                 ref double xmam,   double tsince)
      {
         ...
         else
         {
            ...
            xls    = xls + dls;
            xnode = GeoMath.AcTan(alfdp, betdp);
            xmam   = xmam + pl;
      ...
      /// orbit model.
      /// </remarks>
      public override TimedMotionState GetPosition(double tsince)
      {
         ...
         double a = Math.Pow(OrbitConsts.Xke / xn, 2.0 / 3.0) * GeoMath.Sqr(tempa);
         double e    = em - tempe;
         ...
         xn = OrbitConsts.Xke / Math.Pow(a, 1.5);

...
system identifier.
    ...
    public struct SatelliteNumber : IComparable<SatelliteNumber>, IEquatable<SatelliteNumber>
    {
        ...
        /// 默认编号通常为 U00.
        /// </summary>
        public static SatelliteNumber Default = new SatelliteNumber(00, Gnsser.SatelliteType.U);
        /// <summary>
        /// 形如 G01 字符串
        /// </summary>
        /// <returns></returns>
        ...
        public static SatelliteNumber Parse(string snn)
        {
            ...
            try
            {
                ...
                //无标识符默认为GPS
                if (snn.StartsWith(" ") || snn.Length == 2 || snn.Length == 1) s.SatelliteType = SatelliteType.G;
                else
                {
                    var type = snn.Substring(0, 1).ToUpper();
                    if (!SatelliteTypeHelper.SatTypes.Contains(type)) { s.SatelliteType = SatelliteType.U; }
                    else { s.SatelliteType = (SatelliteType)Enum.Parse(typeof(SatelliteType), type); }
                }
                //解析编号
                var str = snn;
                ...
                else if (Geo.Utils.IntUtil.IsInt(str))
                {
                    s.PRN = int.Parse(str);
                }
                return s;
        ...
        /// 返回默认32个GPS卫星编号
        /// </summary>
        public static List<SatelliteNumber> DefaultGpsPrns { get { return GetPrns(Gnsser.SatelliteType.G, 32); } }
        /// <summary>
        /// 返回默认35个北斗卫星编号序列
        /// </summary>
        public static List<SatelliteNumber> DefaultBeidouPrns { get { return GetPrns(Gnsser.SatelliteType.C, 35); } }
        /// <summary>
        /// 所有可能的卫星编号，四大系统
        /// </summary>
        ...
        private static List<SatelliteNumber> GetAllPrns() { 
               var allPrns = new List<SatelliteNumber>(); 
                ...
                allPrns.AddRange(GetPrns(Gnsser.SatelliteType.R, 32));
                allPrns.AddRange(GetPrns(Gnsser.SatelliteType.E, 36));  
            return allPrns;
        }
        /// <summary>

...
//2018.11.07, czs, create in hmx, 无电离层双差结果转换
...
namespace Gnsser
{
    ...
    /// 无电离层双差结果转换
    /// </summary>
    public class IonoFreeNetDoubleDifferResultReader
    {
        /// <summary>
        /// 读取到历元存储对象
        /// </summary>
        ...
        {
            var tables = ReadAmbiResultInCycle(inputPathes, isRemoveCsPrn);
        ...
        /// <returns></returns>
        public static ObjectTableManager ReadAmbiResultInCycle(string[] inputPathes, bool isRemoveCsPrn = true)
        {
            return ReadAmbiResult(inputPathes,  isRemoveCsPrn, false, Frequence.GpsL1.WaveLength);
        }
        /// <summary>
        /// 读取模糊度结果,单位：米
        /// </summary>
        ...
        /// <param name="isRemoveCsPrn"></param>
        /// <returns></returns>
        public static ObjectTableManager ReadAmbiResultInDefaultUnit(string[] inputPathes, bool isRemoveCsPrn)
        {
            ...
            foreach (var table in smartTalbes)
            {
                ...
                //colToRemove.Add(table.GetIndexColName());
                foreach (var name in table.ParamNames)
                {
                    if(!name.Contains(ParamNames.AmbiguityLen)
                        && !name.Contains("位N")
                        && !name.Contains(table.GetIndexColName())
                        )
                    {
                        colToRemove.Add(name);
                    } 
                }

                table.RemoveCols(colToRemove);

                //table.RemoveCols(new string[] { "CycleSlipe", Gnsser.ParamNames.Du, Gnsser.ParamNames.De, Gnsser.ParamNames.Dn, Gnsser.ParamNames.Dx, Gnsser.ParamNames.Dy, Gnsser.ParamNames.Dz, "cDt_r", "C*dtr", "StdDev", "WetTrop" }); // 删除不参与计算的无关列

...
//2015.04.16, czs, edit in namu, 字符串不足指定长度时，就不再截取，李林阳读取IGS abmf1260.14o 数据时发现该问题。
...
//2018.08.15, czs, edit in hmx, StateFlag 扩展为2位，支持GNSSer自定义
...
namespace Gnsser.Data.Rinex
{
    ...
    {
        ILog log = Geo.IO.Log.GetLog(typeof(RinexObsFileReader));
        /// <summary>
        /// 初始化一个读取器。
        /// </summary>
        ...
        public RinexObsFileReader(string fileName, bool IsReadContent = true, int MaxLenOfMarkerName = 8, bool isReplaceSpaceAnddMinusMaker=true)
        {
            ...
            if (Geo.Utils.FileUtil.IsValid(fileName))
            {
        ...
        /// 是否替换“-”和“ ”用“_”
        /// </summary>
        public int IsReplaceSpaceAnddMinusMaker { get; set; }
        /// <summary>
        /// 读取区间
        /// </summary>
        ...
        /// </summary>
        /// <returns></returns>
        public RinexObsFileHeader GetHeader()
        {
       ...
       static object locker = new object(); 
        ...
        /// <returns></returns>
        public  RinexEpochObservation Get(Geo.Times.Time gpsTime, double toleranceSeccond = 1e-15)
        {
            try
            {
        ...
        /// <param name="toleranceSeccond"></param>
        /// <returns></returns>
        private RinexEpochObservation LoopAndFind(Geo.Times.Time time, int totalLoopCount, double toleranceSeccond)
        {
            //不在有效时段内。
        ...
        {
            if (!Geo.Utils.FileUtil.IsValid(rinexFilePath)) { return null; }
            ...
            //暫時這樣
            Setting.GnsserConfig.GlonassSlotFrequences = header.GlonassSlotFreqNums;
        ...
        public static string CheckAndSetNavPath(string rinexFilePath, RinexObsFileHeader header, string postFix)
        {
            ...
            if (Geo.Utils.FileUtil.IsValid(navPath)) { header.NavFilePath = navPath; }
            else
            {
                ...
                if (Geo.Utils.FileUtil.IsValid(navPath))
                {
                ...
                case RinexHeaderLabel.GLONASS_SLOT_FRQ://3.03
                    {
                        ...
                        var strs = Geo.Utils.StringUtil.Split(str0, 7);
                ...
                case RinexHeaderLabel.GLONASS_COD_PHS_BIS://3.03
                    {
                        ...
                        var strs = Geo.Utils.StringUtil.Split(str0, 13);
                        foreach (var s in strs)
                        {
                            if (String.IsNullOrWhiteSpace(s)) { continue; }
                            var subStrs = s.Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
        ...
        public static string BuildSiteName(string makerName, int MaxLenOfMarkerName=8, string replacerChar ="0")
        {
            ...
            makerName = Geo.Utils.StringUtil.SubString(makerName, 0, MaxLenOfMarkerName).ToUpper();
            return makerName;
        }
        ...
        private List<SatelliteNumber> ReadEpochHeaderV2(StreamReader reader, string line, ref RinexEpochObservation rec)
        {
            ...
            if (EpochFlag == 4)//头部信息单独处理
            {
                ...
                //读完后再读一行为下一历元头部内容
                line = reader.ReadLine();
                EpochFlag = int.Parse(line.Substring(27, 2));//扩展为2位，支持GNSSer自定义
        ...
        public RinexEpochObservation ReadEpochObservationV2(RinexObsFileHeader header, StreamReader reader, string line, bool IsReadContent = true, bool skipError = true)
        {
            ...
            else
            {
                ...
                foreach (SatelliteNumber satNum in prns)//挨个卫星读取
                {
                    RinexSatObsData s = ReadObservationDataV2(CurrentObsCodesV2, reader, satNum, conments);
        ...
        /// <returns></returns>
        public RinexSatObsData ReadObservationDataV2(List<string> obsCodes, StreamReader reader, SatelliteNumber satNum, List<String> comments = null)
        {
            string line = null;
            ...
            //{
                ...
                var obsTypeCount = obsCodes.Count;// header.GetObsTypesCount(satNum.SatelliteType);
        ...
        {//扩展为2位，支持GNSSer自定义
            ...
            while (EpochFlag != 0 && EpochFlag % 10 != 8 && EpochFlag % 10 != 6) //8 结尾是钟跳， 6为周跳
            {
                ...
                EpochFlag = int.Parse(line.Substring(30, 2)); //扩展为2位，支持GNSSer自定义
            ...
            //第一行 
            rec.ReceiverTime = Time.Parse(line.Substring(2, 27));//时间项目有可能是不存在的，这里要判断,解析函数具有判断功能。
            rec.EpochFlag = EpochFlag;
            SatCount = int.Parse(line.Substring(32, 3));
        ...
        public  RinexSatObsData ParseObservationLineV3(RinexObsFileHeader header, string line, bool isReadContentLine = true, bool ignoreEmpty = false)
        {
            ...
            List<string> types = header.GetOrInitObsCodes(satNum.SatelliteType);
        ...
        /// <returns></returns>
        private  RinexObsValue ParseObsValueV2V3(string obsStr, string obsCode)
        {
            if (obsStr.Trim().Length == 0) return new RinexObsValue(0, obsCode);

            double val = Geo.Utils.StringUtil.ParseDouble(StringUtil.SubString(obsStr, 0, 14), 0);
        ...
        /// <param name="isFormateName"></param>
        /// <returns></returns>
        public static RinexObsFileHeader ReadHeader(string path, int markerCount = 8, bool isFormateName = true)
        {
        ...
        /// <param name="p"></param>
        /// <returns></returns>
        internal static double GetRatioOfSatCount(string inpath, double satCount )
        {
            int count = 0;
            int all = 0;

...
//2018.11.02, czs, edit in hmx, 独立提出，用于计算网解
...
namespace Gnsser.Winform
{
    ...
    /// 多GNSS文件网解计算器。
    /// </summary>
    public partial class MultiSiteNetSolveForm : ObsFileAdjustStreamForm
    {
        Log log = new Log(typeof(MultiSiteNetSolveForm));
        public MultiSiteNetSolveForm()
        {
            InitializeComponent();
            this.SetExtendTabPageCount(2, 5);
            InitSolverType<MultiSiteNetSolverType>();
        }

        #region 属性
        protected override IFileGnssSolver GnssSolver { get; set; }
        MultiSiteBackGroundRunner Runner { get; set; }
        ...
        protected override void Run(string[] inputPathes)
        {
            ...
            Runner = new MultiSiteBackGroundRunner(this.Option, inputPathes);
        ...
        private void button_drawDop_Click(object sender, EventArgs e)
        {
            ...
            {
                Geo.Utils.FormUtil.ShowWarningMessageBox("请在输出中选中DOP，计算后再试");
            }

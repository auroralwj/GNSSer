...
{
    public partial class BaseLineNetSolverForm : Form, IShowLayer
    {
        Log log = new Log(typeof(BaseLineNetSolverForm));
        public event ShowLayerHandler ShowLayer;
        public BaseLineNetSolverForm()
        {
            InitializeComponent();
            this.GetOrCreateOption();
            this.Init();
        }
        public BaseLineNetSolverForm(string projectDirectory)
        {
            InitializeComponent();
        ...
        private void 清空ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (Geo.Utils.FormUtil.ShowYesNoMessageBox("将清空所有测站及基线，请确保基线结果已经保存,\r\n 确定移除？") == System.Windows.Forms.DialogResult.Yes)
            {
                ...
                this.richTextBoxControl_currentLine.Clear();
            }
        }
        ...
        {
            var path = resultFileNameBuilder.BuildFinalResultFilePath(typeof(EpochDoubleDifferPositionResult));
            if (!File.Exists(path))
            {
                var pathes = Directory.GetFiles(this.SolverDirectory, "*.BaseLine.txt.xls");
                if (pathes.Length > 0)
                {
                    path = pathes[0];
                }
        ...
        private void SolveBaseLine(List<SiteObsBaseline> sitebaseLines)
        {
            ...
            this.GetOrCreateOption();
            this.UiToEntity();
            this.Option.OutputDirectory = this.SolverDirectory;
        ...
        private void 打开残差文件RToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            var path = Path.Combine(SolverDirectory, resultFileNameBuilder.BuildEpochResidualFileName(line.LineName.Name));
        ...
        {
            var line = this.GetCurrentBaseLine(); if (!Check(line)) { return; } 
            var path = Path.Combine(this.SolverDirectory, resultFileNameBuilder.BuildEpochParamFileName(line.LineName.Name));
            TryOpenTableForm(path);
        }
        ...
        {
            var line = this.GetCurrentBaseLine(); if (!Check(line)) { return; }
            var path = Path.Combine(SolverDirectory, resultFileNameBuilder.BuildEpochParamRmsFileName(line.LineName.Name));
            TryOpenTableForm(path);
        }
        ...
        private void 查看历元收敛图EToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            var path = Path.Combine(SolverDirectory, resultFileNameBuilder.BuildEpochParamFileName(line.LineName.Name));
        ...
        private void SaveAsLeoAsc(BaseLineNet BaseLineNet, string name)
        {
            ...
            var outpath = Path.Combine(SolverDirectory, name + Setting.BaseLineFileOfLgoExtension);
        ...
        private void listBox_vector_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            //当前基线残差图
            var path = "";
            path = Path.Combine(this.SolverDirectory, resultFileNameBuilder.BuildEpochResidualFileName(currentLine.LineName.Name));
            if (File.Exists(path))
            {
                var table = ObjectTableReader.Read(path);
            ...
            //当前基线参数收敛图
            path = Path.Combine(SolverDirectory, resultFileNameBuilder.BuildEpochParamFileName(currentLine.LineName.Name));
            ...
            if (File.Exists(path))
            {
                ...
                commonChartControl_currentParamConvergence.DataBind(phaseObsTable);
            }
        }
        ...
        private void 查看当前测站PPP收敛图VToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            var path = Path.Combine(SolverDirectory, resultFileNameBuilder.BuildEpochParamFileName(site.SiteObsInfo.FileInfo.FileName));
            var paramNames = new List<string>() { ParamNames.Epoch, ParamNames.De, ParamNames.Dn, ParamNames.Du };
            GnsserFormUtil.ShowChartForm(path, paramNames);
        }
        ...
        private void pPP计算并更新当前头文件PToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            var pathes = new List<ObsSiteInfo> { site };
        ...
        private void RunPpp(List< ObsSiteInfo> pathes)
        {
            var option = GnssProcessOption.GetDefaultIonoFreePppOption();
            PppRinexFileApproxUpdater runner = new PppRinexFileApproxUpdater(option, IsReplaceApproxCoordWhenPPP);
             
            runner.Update(pathes);
        }
        #endregion

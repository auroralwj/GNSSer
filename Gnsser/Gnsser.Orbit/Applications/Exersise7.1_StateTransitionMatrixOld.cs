...
//2017.06.22, czs, funcKeyToDouble from c++ in hongqing, Exersise7.1_StateTransitionMatrix
using System;
...
using Geo.Algorithm; using Geo.Utils; namespace Gnsser.Orbits
{
   ...
   public class  AuxParam2
    ...
    //  Exersise7.1_StateTransitionMatrix
    class Exersise7_1_StateTransitionMatrixOld
    {
         // Flattening; WGS-84 
        ...
        public static double Max(Matrix M)
        {
            ...
            for (int i = 0; i < n; i++)
                for (int j = 0; j < m; j++)
                    if (Math.Abs(M[i, j]) > max) max = Math.Abs(M[i, j]);
        ...
        public static Matrix InvSymp(Matrix A)
        {
            int N = A.RowCount;
            int n = N / 2;

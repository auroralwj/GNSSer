...
namespace Geo.Draw
{
    ...
    /// 二维颜色绘图
    /// </summary>
    public partial class TwoDimColorDrawControlOld : UserControl
    {
        /// <summary>
        /// 默认构造函数
        /// </summary>
        public TwoDimColorDrawControlOld()
        {
            InitializeComponent();

            Init(this.pictureBox1.Size);
        }
        ...
        /// 颜色生成器
        /// </summary>
        public ThreeStepColorBuilder ColorBuilder { get; set; }
        /// <summary>
        /// 颜色图例尺寸
        /// </summary>
        ...
        public void Draw(List<GeoCoord> geoCoords)
        {
            if (ChartSize.Width == 0 || ChartSize.Height == 0) { return; }

            Bitmap bmp = new Bitmap(ChartSize.Width, pictureBox1.Size.Height); ;
        ...
        /// <returns></returns>
        public Bitmap Draw(TwoNumeralKeyAndValueDictionary table)
        {
            ...
            this.ColorBuilder = new ThreeStepColorBuilder(table.ValueSpan, Color.Blue, Color.Cyan, Color.Yellow, Color.Red);
        ...
        private void DrawYLabels()
        {
            ...
            for (var y = this.TableValue.KeyBSpan.Start; y <= TableValue.KeyBSpan.End; y = TableValue.IntervalB + y)
            {
                ...
                if ((pos.Y - prevYPixe >= minYDiffer && endY - pos.Y > minYDiffer) || Math.Abs(y - this.TableValue.KeyBSpan.End) < 1E-8)
                {
        ...
        private void DrawXLables()
        {
            ...
            for (var x = TableValue.KeyASpan.Start; x <= this.TableValue.KeyASpan.End; x += TableValue.IntervalA)
            {
                ...
                if (((pos.X - prevX) >= minXDiffer && endX - pos.X > minXDiffer) || Math.Abs(x - this.TableValue.KeyASpan.End) < 1E-8)
                {
                    prevX = pos.X;
        ...
        {
            if (UserToScreenCoordConverter != null)
            {
        ...
        private void TwoDimColorDrawControl_Resize(object sender, EventArgs e)
        {
            Init(this.pictureBox1.Size);
        }
    }

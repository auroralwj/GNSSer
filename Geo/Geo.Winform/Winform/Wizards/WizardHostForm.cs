...
{
    public partial class WizardHostForm : Form
    {
        private const string VALIDATION_MESSAGE = "当前页面无效，请完成所需的配置。";
        ...
        public WizardHostForm()
            :this(new WizardPageCollection())
        {
        }
        public WizardHostForm(WizardPageCollection WizardPageCollection)
        {
            InitializeComponent();
            WizardPages = WizardPageCollection;
            WizardPages.WizardPageLocationChanged += new WizardPageLocationChangedEventHanlder(WizardPages_WizardPageLocationChanged);
        }
        ...
        public void LoadNextPage(int pageIndex, int previousPageIndex, bool savePreviousPage)
        {
            if (pageIndex != -1)
            {
                ...
                WizardPages[pageIndex].LoadPage();
                UpdateNavigation();
            }
        }

...
//2016.08.04, czs, edit in 福建 永安, 修改，重命名委托
using System;
    ...
    public partial class EnumRadioControl : UserControl
    {
        ...
        public void Init<TEnum>(bool IsCheckOne = true)
        {
            ...
            List<string> availableNames = new List<string>();
                var enumTypeStr = typeof(TEnum).ToString();

            if ((Geo.Setting.VersionType == VersionType.Public || Geo.Setting.VersionType == VersionType.BaselineNet) && enumTypeStr.Contains("SolverType"))
            {
                int ii = 0;
                if(enumTypeStr.Equals("Gnsser.SingleSiteGnssSolverType", StringComparison.CurrentCultureIgnoreCase))
                {
                    availableNames.Add("无电离层组合PPP");
                    availableNames.Add("最简伪距定位");
                }else if(enumTypeStr.Equals("Gnsser.MultiSiteNetSolverType", StringComparison.CurrentCultureIgnoreCase))
                {
                    if (Geo.Setting.VersionType == VersionType.BaselineNet)
                    {
                        availableNames.Add("网解双差定位");
                    }
                }else if(enumTypeStr.Equals("Gnsser.TwoSiteSolverType", StringComparison.CurrentCultureIgnoreCase))
                {
                    availableNames.Add("无电离层双差");
                    availableNames.Add("单历元单频双差");
                    if (Geo.Setting.VersionType == VersionType.BaselineNet)
                    {
                        availableNames.Add("单历元双频双差");
                        availableNames.Add("单历元载波双差");
                        availableNames.Add("多历元载波双差");
                    }
                }
            }
            else
            {
                availableNames.AddRange(names);
            }
            int i = 0;
        ...
        {
            Remove<TEnum>(Geo.Utils.EnumUtil.GetOthers<TEnum>(enums));
        }
        ...
        public void Disable<TEnum>(IEnumerable<TEnum> enums)
        {
            foreach (var item in this.flowLayoutPanel1.Controls)
            {
                ...
                {
                    var btn = item as RadioButton;
                    var type = Geo.Utils.EnumUtil.Parse<TEnum>(btn.Text);

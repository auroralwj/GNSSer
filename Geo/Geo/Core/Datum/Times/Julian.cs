...
namespace Geo.Times
{
   ...
   public class Julian
   {
      ...
      public const double SecPerDay = 86400.0;       // Fraction per day (solar)
      ...
      /// (Theta GMST).
      /// </returns>
      public double GetGreenwichMeanSiderealTime(bool isRad = true) 
      {
         // References:
         ...
         GMST = (GMST + SecPerDay * OmegaE * UT) % SecPerDay;
   
         if (GMST < 0.0)
         {
            GMST += SecPerDay;  // "wrap" negative modulo value
         }

         double angle = (GMST / SecPerDay);
         if (isRad)
         {
             angle = (TwoPi * angle);
         }
      ...
      /// the given longitude.
      /// </returns>
      public double GetLocalMeanSiderealTime(double lon, bool isDegree  =false)
      {
          if (isDegree)
              lon *= CoordConsts.DegToRadMultiplier;

         double result = (GetGreenwichMeanSiderealTime() + lon) % TwoPi;

         if (isDegree)
             result *= CoordConsts.RadToDegMultiplier;
         return result;
      }

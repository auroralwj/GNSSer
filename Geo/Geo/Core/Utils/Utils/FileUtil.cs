...
namespace Geo.Utils
{
    ...
    public static class FileUtil
    {
        ...
        public static void SetFileAssociation(string exePath, string extName, string typeName, string contentType)
        {
            RegistryKey reg = Registry.ClassesRoot.CreateSubKey(extName);//定义文件关联注册表项的属性 
            reg.SetValue("", typeName);
            reg.SetValue("Content Type", contentType);
            reg = reg.CreateSubKey("shell\\open\\command");
            reg.SetValue("", exePath + " %1");
            reg.Close();
        }
        ...
        public static void TryDeleteFileOrDirectory(string fileOrDirectory)
        {
            ...
            {
                new Geo.IO.Log(typeof(FileUtil)).Error(fileOrDirectory + "删除失败！ " + ex.Message);
            }
        }
        /// <summary>
        /// 删除文件或目录
        ...
        public static void TryDeleteFile(string filePath)
        {
            ...
            {
                new Geo.IO.Log(typeof(FileUtil)).Error(filePath + "删除失败！ " + ex.Message);
            }
        }

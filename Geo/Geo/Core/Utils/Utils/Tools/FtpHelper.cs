//2015.11.13, czs, create in hongqing, FTP帮助类，源于网上，稍作修改
...
namespace Geo.Utils
{
     ...
     public class FtpHelper
    {
        ...
        //基本设置
        private string path { get; set; }    //目标路径
        private string ftpip { get; set; }     //ftp IP地址
        private string username { get; set; }   //ftp用户名
        private string password { get; set; }    //ftp密码
        ...
        /// 从ftp服务器上获得文件列表
        /// 如何获取某一目录下的文件和文件夹列表。
        /// 由于FtpWebRequest类只提供了WebRequestMethods.Ftp.ListDirectory方式和WebRequestMethods.Ftp.ListDirectoryDetails方式。
        /// 这个方法获取到的是包含文件列表和文件夹列表的信息。并不是单单只包含某一类。
        /// 为此我们需要分析获取信息的特点。分析发现，对于文件夹会有“<DIR>”这一项，而文件没有。
        /// 所以我们可以根据这个来区分。一下分别是获取文件列表和文件夹列表的代码：
        /// 获取文件夹：
        /// </summary>
        /// <param name="RequedstPath">服务器下的相对路径</param>
        /// <returns></returns>

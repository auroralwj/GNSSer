...
namespace Geo.Algorithm
{
    ...
    /// </summary>
    [Serializable]
    public class Vector : Geo.Algorithm.AbstractVector, ICloneable, IReadable
    {
        #region 构造函数
        /// <summary>
        /// 构建一个二维向量
       ...
       /// <param name="values"></param>
        ...
        public static bool IsValid(IVector vector)
        {
            ...
            foreach (var item in vector)
            {
                if (!Geo.Utils.DoubleUtil.IsValid(item)) { return false; }
            }
            return true;
        }
        /// <summary>
        ...
        public virtual string ToReadableText(string splitter="\n")
        {
            ...
            foreach (var item in this)
            {
                ...
                sb.Append(Geo.Utils.DoubleUtil.ToReadableText(item)); 
                i++;
            }
            sb.AppendLine();
        ...
        public static Vector Parse(string[] items, string [] splitter = null)
        {
            ...
            {
                vector[vecIndex] = Geo.Utils.DoubleUtil.TryParse(items[lineIndex], 0);
            } 

            vector.ParamNames = Geo.Utils.StringUtil.ExtractNames(items, paramNameMarker, splitter);//  paramNames;

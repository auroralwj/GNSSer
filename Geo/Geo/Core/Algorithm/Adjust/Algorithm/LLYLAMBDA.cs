// 2015.06.16, lly, create in zz, LAMBDA法
//2016.03.22, czs, edit, 部分名称重构
...
namespace Geo.Algorithm
{
    ...
    {     
        ...
        public LlyLambda(int n, double[][] Q, double[] a)
        {
            this.NumberOfFloatParam = n;
            this.Q = new ArrayMatrix(Q);
            this.FloatParams = a;
            this.FixingNumber = 2;
        ...
        /// <param name="F">固定解向量。fixed solutions (n * m)</param>
        /// <param name="s">固定解的残差平方和，便于ratio检验。sum of squared residulas of fixed solutions (1 * m)</param>
        public LlyLambda(int n, int m, double[][] Qa, double[] a)
        {
            //
            this.NumberOfFloatParam = n;
            this.FixingNumber = m;
            this.CovaOfParams = Qa;
            this.FloatParams = a;
        ...
        public int getLambda(ref double[] F, ref double[] s)
        {
            ...
            if (NumberOfFloatParam <= 0 || FixingNumber <= 0) info = -1;
            double[] L, D, Z, z, E;
            L = new double[NumberOfFloatParam * NumberOfFloatParam];
            D = new double[NumberOfFloatParam * 1];
            Z = new double[NumberOfFloatParam * NumberOfFloatParam]; for (i = 0; i < NumberOfFloatParam; i++) Z[i + i * NumberOfFloatParam] = 1;
            z = new double[NumberOfFloatParam * 1];
            E = new double[NumberOfFloatParam * FixingNumber];

            double[] Q = new double[NumberOfFloatParam * NumberOfFloatParam];
            for (i = 0; i < NumberOfFloatParam; i++)
            {
                for (j = 0; j < NumberOfFloatParam; j++)
                {
                    Q[i * NumberOfFloatParam + j] = CovaOfParams[i][j];
                }
            }

            //if (!(info = LD(n, Q, ref L, ref D))) { }
            if ((info = LD(NumberOfFloatParam, Q, ref L, ref D)) != -1) //说明分解成功
            {
                //lambad reduction 
                reduction(NumberOfFloatParam, ref L, ref D, ref Z);

                //z=Z'*a;

                char[] tr = new char[2]; tr[0] = 'T'; tr[1] = 'N';
                matmul(tr, NumberOfFloatParam, 1, NumberOfFloatParam, 1.0, Z, FloatParams, 0.0, ref z);


                //mlambda search
                if ((info = msearch(NumberOfFloatParam, FixingNumber, ref L, ref D, ref z, ref E, ref s)) != -1)
                {
                    info = solve(tr, Z, E, NumberOfFloatParam, FixingNumber, ref F); //  F=Z'\E
                }
        ...
        private int msearch(int n, int m, ref double[] L, ref double[] D, ref double[] zs, ref double[] zn, ref double[] s)
        {
            ...
            k = n - 1; dist[k] = 0.0;
            zb[k] = zs[k];
            z[k] = Math.Floor(zb[k] + 0.5); y = zb[k] - z[k]; step[k] = Sign(y);
            for (c = 0; c < MaxLoop; c++)
            {
        ...
        private int solve(char[] tr, double[] A, double[] Y, int n, int m, ref double[] X)
        {
            ...
            if ((info = InverseMatrix(ref B, n)) != -1)
        ...
        /// <param name="n"></param>
        /// <returns></returns>
        private int InverseMatrix(ref double[] A, int n)
        {
            double d = 0;
            int i, j;
            int[] indx = new int[n];
            ...
            if (LuDecomposition(ref B, n, ref indx, ref d) == -1) { return -1; }
            ...
            for (j = 0; j < n; j++)
            {
                ...
                LuBackSubstitution(B, n, indx, ref tmp);
        ...
        /* LU decomposition ----------------------------------------------------------*/
        private int LuDecomposition(ref double[] A, int n, ref int[] indx, ref double d)
        {
            double big, s, tmp;
        ...
        private void LuBackSubstitution(double[] A, int n, int[] indx, ref double[] b)
        {
            double s;
            int i, ii = -1, ip, j;
        ...
        public bool process()
        {
            ...
            ArrayMatrix Zti = new ArrayMatrix(NumberOfFloatParam, NumberOfFloatParam); for (int i = 0; i < NumberOfFloatParam; i++) Zti[i, i] = 1; // initialize Z to a unit matrix
            ...
            double[] vl = new double[NumberOfFloatParam]; // work array of length n

            double[] ak = new double[NumberOfFloatParam]; // contains the (integer) increments of the original float ambiguities

            for (int i = 0; i < NumberOfFloatParam; i++)
            {
                //
                ak[i] = Math.Truncate(FloatParams[i]);

                //   double kkk = Math.Round(a[i]);

                vl[i] = FloatParams[i] - ak[i];

                FloatParams[i] = vl[i];
            }

            //make the L_1^{-T} D_1^{-1} L_1^{-1} decomposition of the covariance matrix Q

            //case 1:
            double[] D = new double[NumberOfFloatParam];
            ArrayMatrix L = new ArrayMatrix(NumberOfFloatParam, NumberOfFloatParam);
            info = LTDL(D, L);
            ...
            //Compute the Z-transformation based on L and D of Q, ambiguities are transformed according to Z^T.
            ...
            for (int i = 0; i < NumberOfFloatParam; i++) D[i] = 1.0 / D[i];

            //find a suitable Chi^2 such that we have two candidates an minimum:
            //use an eps to make sure the two candidates are inside the ellipsoid some small number
            double eps = Math.Pow(10, -6);
            double[] v3 = Chistart(D, L);

            double Chi_1 = v3[1] + eps;
            //find the two candidates with minimun norm
            ArrayMatrix cands = new ArrayMatrix(NumberOfFloatParam, MaxCan); // 2-dimensional array to store the candidates
            ArrayMatrix disal1 = new ArrayMatrix(1, MaxCan); //according squared norms 
            int ipos = 0;
            int ncan = 0;

            info = Search(Chi_1, MaxCan, NumberOfFloatParam, D, L, FloatParams, ref disal1, ref cands, ref ipos, ref ncan);
            if (info == false) return info;
            ...
            {
                FloatParams[i] = 0.0;
                for (int j = 0; j < NumberOfFloatParam; j++)
                {
                    FloatParams[i] += Zti[i, j] * cands[j, ipos];
                }
                FloatParams[i] = FloatParams[i] + ak[i];
            }
        ...
        private bool Search(double Chic, int MaxCan, int n, double[] D, ArrayMatrix L, double[] a, ref  ArrayMatrix disal1, ref ArrayMatrix cands, ref int ipos, ref int ncan)
        {
            ...
            while (ende == false)
            {
                ...
                //idst[i] is the distance of this left most integer to the a_hat
        ...
        {
            for (int i = 0; i < NumberOfFloatParam; i++) cands[i, ipos - 1] = dist[i];
            disal1[0, ipos - 1] = t;
            tmax = t;
            imax = ipos - 1;
        ...
        private void Backtrack(ref bool ende, double[] left, ref int i, double[] lef, double[] dist, double[] endd)
        {
            ...
            while (c_stop == false && i < NumberOfFloatParam - 1)
            {
                i = i + 1;
                if (dist[i] <= endd[i])
                {
                    ...
                    c_stop = true;
                    if (i == NumberOfFloatParam - 1)
                    { cand_n = true; }
                }
            }
            if (i == NumberOfFloatParam - 1 && cand_n == false)
            { ende = true; }
        }
        ...
        private double[] Chistart(double[] D, ArrayMatrix L)
        {
            ...
            double[] dist = new double[NumberOfFloatParam];
            double[] e = new double[NumberOfFloatParam];
            double[] t = new double[NumberOfFloatParam];

            for (int i = 0; i < NumberOfFloatParam; i++) dist[i] = Math.Round(FloatParams[i]) - FloatParams[i];

            for (int i = 0; i < NumberOfFloatParam; i++)
            {
                for (int j = i; j < NumberOfFloatParam; j++)
                {
                    e[i] += L[j, i] * dist[j];
                }
            }
            ...
            double t_0 = 0;
            for (int i = 0; i < NumberOfFloatParam; i++) t_0 = t_0 + D[i] * e[i] * e[i];
            ...
            while (sw == true)
            {
                ...
                while (sw == false && i > 0)
                {
                    ...
                    if (delta < D[i + 1])
                    {
                        ...
                        for (int j = i + 2; j < NumberOfFloatParam; j++) // to be coded effectively: interchange of columns L[i+2:n,i] and L[i+2:n,i+1]
                        {
                            temp = L[j, i];
                            L[j, i] = L[j, i + 1];
                            L[j, i + 1] = temp;
                        ...
                        for (int j = 0; j < NumberOfFloatParam; j++) // to be coded effectively: swap Zti[1:n,i] with Zti[1:n,i+1]
                        {
                            temp = Zti[j, i];
                            Zti[j, i] = Zti[j, i + 1];
                            Zti[j, i + 1] = temp;
                        ...
                        temp = FloatParams[i];
                        FloatParams[i] = FloatParams[i + 1];
                        FloatParams[i + 1] = temp;
            ...
            for (int i = last; i >= first; i--)
            {
                for (int j = i + 1; j < NumberOfFloatParam; j++)
                {
                    ...
                    if (mu != 0)
                    {
                        ...
                        for (int s = 0; s < NumberOfFloatParam; s++) Zti[s, j] = Zti[s, j] + mu * Zti[s, i];
        ...
        /// </summary>
        public int NumberOfFloatParam { get; set; }

        /// <summary>
        /// 备选的值，通常为两组，一组最优，一组次优，便于RATIO判别
        /// </summary>
        public int FixingNumber { get; set; }
        /// <summary>
        /// variance covariance matrix(symmetric)
        /// </summary>
        public ArrayMatrix Q { get; set; }
        ...
        /// </summary>
        public double[] FloatParams { get; set; }

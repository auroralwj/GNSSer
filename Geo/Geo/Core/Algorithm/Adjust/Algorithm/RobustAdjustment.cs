    ...
    public class RobustAdjustment
    {
      ...
      //  public List<int[]> totalBeginOfBaselineList = new List<int[]>();
       ...
       // public List<int[]> totalEndOfBaselineList = new List<int[]>();
        ...
        public RobustAdjustment(AdjustBasicInformation BasicInfor,
            ...
            double[][] ppList)
        {
            ...
            this.totalUnknowPointnumber = BasicInfor.UnknowPointnumber;
            this.totalBaselineNumber = BasicInfor.TotalBaselineNumber;
            this.totalPointName = BasicInfor.TotalPointName;
            this.totalKnownPointName = BasicInfor.KnownPointName;
            // this.pointsXYZ = totalPointXYZ;
            //int n = totalPointXYZ.Length / 3;
            //for (int i = 0; i < n; i++)
            //{
            //    XYZ xyz = new XYZ(totalPointXYZ[3 * i + 0], totalPointXYZ[3 * i + 1], totalPointXYZ[3 * i + 2]);
            //    this.pointsXYZ.Add(xyz);
            //}
          //  pointsXYZ = totalPointXYZ;
            pointsXYZ = new double[totalPointXYZ.Length];
            totalPointXYZ.CopyTo(pointsXYZ, 0);
            //
        ...
        /// 最小二乘平差函数
        /// </summary>
        public void Parallel_LeastSquares()
        {
            int t = 3 * totalPointNumber;
            int tt = t * (t + 1) / 2;
            ATPA = new double[tt];
        ...
        public void parallelRobust(FactorYype fname, double eps, double k0, double k1, int m)
        {
            ...
            //for (int i = 0; i < FileNumber; i++)
            {
                ...
                Pi.CopyTo(PListBackup[i], 0);//将观测值的权备份到数组PPList
            ...
            while (IsStill)
            {
                ...
                // for (int i = 0; i < FileNumber; i++)
                {
                    ...
                    //残差的权矩阵要时刻更新，新的残差权矩阵=P_last-A*(Q_X)*AT
                });
                ...
                parallGetQV();//更新残差权矩阵，（此前的法方程矩阵已经求逆，即是QX）

                parallGetUnitV();//标准化残差计算
        ...
        public  void GetLV()
        {
            ...
            for (int i = 0; i < FileNumber; i++)
            {
                ...
                for (int j = 0; j < m; j++)
                {
                    int k1 = BegPoint[j];
                    int k2 = EndPoint[j];
                    Vi[j * 3 + 0] = (pointsXYZ[3 * k2 + 0] - pointsXYZ[3 * k1 + 0]) - Li[3 * j + 0];
                    Vi[j * 3 + 1] = (pointsXYZ[3 * k2 + 1] - pointsXYZ[3 * k1 + 1]) - Li[3 * j + 1];
                    Vi[j * 3 + 2] = (pointsXYZ[3 * k2 + 2] - pointsXYZ[3 * k1 + 2]) - Li[3 * j + 2];
        ...
        {
            int n = pointsXYZ.Length;
            if (n != dXYZ.Length) { throw new Exception("出错！"); }
            for (int i = 0; i < n; i++)
            {
                pointsXYZ[i] += dXYZ[i];       
            }

//2014.07， cy，  转换
//2018.10.27, czs, edit in hmx, 代码整理，重构 为 Decimal
...
{ 
    ...
    /// 借鉴的宋立杰的对称矩阵求逆算法。
    /// </summary>
    public class SljMatrixInverseDecimal
    {
        Log log = new Log(typeof(SljMatrixInverseDecimal));

        /// <summary>Construct a LU decomposition.借用的宋立杰的对称矩阵求逆算法。</summary>	
        public SljMatrixInverseDecimal(ArrayMatrix A)
		{
			LA = A;
			int rows = A.RowCount;
        ...
        public ArrayMatrix Solve(ArrayMatrix B)
		{
            ...
            //仅存下三角元素
            decimal [] lowerTriangular = new decimal[count];
            for (i = 0; i < rows; i++)
            {
                for (j = 0; j <= i; j++)
                {
                    lowerTriangular[i * (i + 1) / 2 + j] = (decimal)LA[i, j];
                }
            }

            var a0 = new decimal[rows];

            int n = rows;

            for (k = 0; k < rows; k++)
            {
                var a00 = lowerTriangular[0];
                if (Math.Abs( a00) == 0)
                {
                ...
                for (i = 1; i < n; i++)
                {
                    var ai0 = lowerTriangular[i * (i + 1) / 2];
                ...
                lowerTriangular[n * (n + 1) / 2 - 1] = 1.0m / a00;
            }
            ...
            for (i = 0; i < rows; i++)
            {
                for (j = 0; j <= i; j++)
                {
                    result[i, j] = (double)lowerTriangular[i * (i + 1) / 2 + j];
                }
            }

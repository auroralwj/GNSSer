...
namespace Geo.Algorithm
{
	...
	{
		public ArrayMatrix U;
		public ArrayMatrix V;
		private double[] s; // singular values
		private int m;
		private int n;
	
		/// <summary>Construct singular value decomposition.</summary>
		public SingularValueDecomposition(ArrayMatrix A)
		{
			ArrayMatrix copy = (ArrayMatrix) A.Clone();
			double[][] a = copy.Array;
			m = A.RowCount;
			n = A.Columns;
			int nu = Math.Min(m,n);
			s = new double [Math.Min(m+1,n)];
			U = new ArrayMatrix(m, nu);
			...
			// Set up the final bidiagonal matrix or order info.
			...
			while (p > 0) 
			{
				...
				// kase = 1     if s(info) and e[k-1] are negligible and k<info
				// kase = 2     if s(k) is negligible and k<info
				// kase = 3     if e[k-1] is negligible, k<info, and s(k), ..., s(info) are not negligible (qr step).
				// kase = 4     if e(info-1) is negligible (convergence).
				for (k = p-2; k >= -1; k--) 
				{
					if (k == -1)
						break;

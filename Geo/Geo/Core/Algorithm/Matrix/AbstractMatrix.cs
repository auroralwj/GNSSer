//2014.09.22, czs, create, 海鲁吐嘎查 通辽, 创建矩阵接口，为后继实现自我的、快速的、大规模的矩阵计算做准备
      ...
      /// <summary>
        ...
        public virtual bool IsSymmetric
        {
            get
            {
                ...
                if (this is DiagonalMatrix) { return true; }
                if (this is SymmetricMatrix) { return true; }
        ...
        {
            IMatrix X = new ArrayMatrix(RowCount, ColCount);
        ...
        public virtual IMatrix Plus(IMatrix right)
        {
            ...
            if (right.ColCount != this.ColCount || this.RowCount != right.RowCount) throw new DimentionException("维数相同才可以计算！");
        ...
        { 
            ...
            var matrix = new ArrayMatrix(array);
            return matrix;
        }
        /// <summary>
        ...
        public virtual IMatrix Multiply(IMatrix right)
        {
            ...
            if (this.ColCount != right.RowCount)
                throw new DimentionException("乘法左边的列必须等于右边的行");

            
            double[][] array = Geo.Utils.MatrixUtil.Multiply(this.Array, right.Array);
            var matrix = new ArrayMatrix(array);
            return matrix;
            ...
            {
                var Right = right as DiagonalMatrix;
                int row = this.RowCount;
                int col = this.ColCount;
                double[] right1 = Right.Vector;
                double[][] dataA = this.Array;
                double[] dataAVector = new double[col];
                double[][] resultArray = new double[row][];
                if (row <= 300)
                {
                    for (int i = 0; i < row; i++)
                    {
                        resultArray[i] = MultiplyRow(dataA[i], right1);
                    }
                }
                else
                {
                ...
                return new ArrayMatrix(resultArray);
            }


            else
            {
                int columns = right.ColCount;
                IMatrix x = new ArrayMatrix(right.Array);
                ArrayMatrix left = new ArrayMatrix(this.Array);

                var result = left * x;

                if (result.ColCount == 1)
                {
                    return new VectorMatrix(result.GetCol(0));
                }
                if (result.IsSquare && this is ArrayMatrix)
                {
                    if (result.IsDiagonal)
                    {
                        return new DiagonalMatrix(result.Array);
                    }
                    if (result.IsSymmetric)
                    {
                        return new SymmetricMatrix(result.Array);
                    }
                }
                return result;
            }
        ...
        public virtual IMatrix Transposition
        {
            get
            {
                IMatrix trans =new ArrayMatrix(ColCount, RowCount);
                int row = this.RowCount;
                int col = this.ColCount;
                for (int i = 0; i < row; i++)
                {
        ...
        {
            return new Geo.Algorithm.ArrayMatrix(this.Array).Inverse;
        }
        /// <summary>
        /// 所有元素求幂
        /// </summary>
        ...
        {
            ArrayMatrix mat = new ArrayMatrix(RowCount, ColCount);
            for (int i = 0; i < RowCount; i++)
            {
                for (int j = 0; j < ColCount; j++)
                {

            ...
            if (arg is double || arg is float)
            {
                ...
                if (IsSuitToString(val))
                {
                    if (ValidCount != validCount)
                    {
                        ...
                        return  Geo.Utils.StringUtil.FillSpaceLeft( val.ToString(pad.PadString), sym.StringWidth);
                    }
                    return Geo.Utils.StringUtil.FillSpaceLeft(val.ToString("0.00000"), sym.StringWidth);
                }
                //整数则直接输出
                if (IsInt(val)) return Geo.Utils.StringUtil.FillSpaceLeft(val.ToString(), sym.StringWidth);

                //采用科学计数法
                return Geo.Utils.StringUtil.FillSpaceLeft(val.ToString("G" + (validCount-1)), sym.StringWidth);//0.12345E9

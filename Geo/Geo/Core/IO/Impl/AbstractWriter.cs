...
namespace Geo.IO
{
    ...
    public abstract class AbstractWriter<TProduct> : AbstractEncodedStreamer, IWriter<TProduct>
    {
        ...
        public AbstractWriter(string path, Encoding encoding) :
            this( Geo.Utils.FileUtil.CheckOrCreateDirectory(Path.GetDirectoryName(path))? new FileStream(path, FileMode.Create, FileAccess.Write):new FileStream(path, FileMode.Create, FileAccess.Write), encoding, Path.GetFileName(path))
        {

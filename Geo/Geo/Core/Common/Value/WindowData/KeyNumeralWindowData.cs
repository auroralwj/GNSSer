...
namespace Geo
{
    ...
    public class NumeralWindowData<TKey> : WindowData<TKey, double> where TKey : IComparable<TKey>
    {
        ...
        /// </summary>
        /// <returns></returns>
        public double LoopDifferValue { get { return Geo.Utils.DoubleUtil.GetLoopDifferValue(this.Values.ToArray()); } }

        /// <summary>
        /// 求绝对值平均
        /// </summary>
        public double AbsAverageValue { get { return Geo.Utils.DoubleUtil.AbsAverage(this); } }
        /// <summary>
        /// 求平均
        /// </summary>
        public double AverageValue { get { return Geo.Utils.DoubleUtil.Average(this); } }
        ...
        public RmsedNumeral Average
        {
            get
            {
                var val = Geo.Utils.DoubleUtil.GetAverageWithStdDev(this);
        ...
        public List<TKey> GetKeysOfGrossError(Func<TKey, double> funcKeyToDouble, int order = 2, double errorTimes = 3, double minWindowSize = 3)
        {
            ...
            foreach (var window in subWindows)
            {
                ...
                {
                    var keys = Geo.Utils.DoubleUtil.GetKeysOfGrossError(item.Data, funcKeyToDouble, errorTimes, order, true);
        ...
        {
            return (Geo.Utils.DoubleUtil.IsGross(this, newVal, errorTimes));
        }
        /// <summary>
        ...
        public NumeralWindowData<TKey> GetNeatlyWindowData(double errorTimes = 3)
        {
            ...
            var dic = Geo.Utils.DoubleUtil.GetNeatlyData<TKey>(this.OrderedData, errorTimes);
            return Create(dic);
        }
        ...
        public NumeralWindowData<TKey> GetNeatlyWindowData(int fitOrder = 2, double errorTimes = 3, bool isLoop = true)
        {
            ...
            var dic = Geo.Utils.DoubleUtil.GetNeatlyData<TKey>(this.OrderedData, this.KeyToDouble, errorTimes, fitOrder, isLoop);
            return Create(dic);
        }
        ...
        public virtual NumeralWindowData<TKey> GetNearstWindowData( TKey key, int count)
        {
            ...
            var keys = Geo.Utils.ComparableUtil.GetNearst<TKey>(orderedKeys, key, count, KeyToDouble);
            Dictionary<TKey, double> dic = new Dictionary<TKey, double>();
        ...
        {
            return Geo.Utils.DoubleUtil.GetKeysOfGrossError(this.OrderedData, errorTimes, loop);
        }
        /// <summary>
        ...
        public virtual RmsedNumeral GetAdaptiveLinearFitValue(TKey newKey, int fitDataCount = int.MaxValue, bool isNeedRms  =true)
        {
            ...
            startIndex = Geo.Utils.IntUtil.GetPositiveOrZero(startIndex);
            var keys = this.OrderedKeys;
            ...
            return Geo.Utils.DoubleUtil.GetWeightedAverage(false, data.ToArray());
        }
        ...
        public int [] FindIndexRange(TKey referKey, int fitDataCount)
        {
            ...
            int currentIndex = Geo.Utils.ListUtil.ClosestToIndexOf(this.OrderedKeys, referKey, KeyToDouble);
            //以当前为中心
        ...
        {
            if (fitDataCount < 4) { fitDataCount = 4; }

            if (marginCount >= fitDataCount / 2 || marginCount < 0) { marginCount = fitDataCount / 4; }

            if (overlapCount >= marginCount || overlapCount < 0) { overlapCount = marginCount / 2; }//默认
        ...
        public PolyfitBuilder BuildPolyfitBuilder(int order, int currentIndex, int polyfitCount, int marginCount = -1)
        {
            ...
            int startIndex = Geo.Utils.IntUtil.GetPositiveOrZero(currentIndex - marginCount);

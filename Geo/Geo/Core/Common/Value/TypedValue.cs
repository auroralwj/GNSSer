//2016.11.25, czs, create in 洪庆, 具有数据类型的值
...
namespace Geo
{
    ...
    public class ValueTypeHelper
    {
        ...
        public static ValueType GetValueType(string val, bool isIntOrFloatFirst = false)
        {
            ...
            {
                if (Geo.Utils.StringUtil.IsNumber(val)) { return ValueType.Long; }
                if (Geo.Utils.StringUtil.IsDecimal(val)) { return ValueType.Float; } 
            }
            else
            {
                if (Geo.Utils.StringUtil.IsDecimal(val)) { return ValueType.Float; }
    ...
    /// 命名的类型
    /// </summary>
    public class NamedType : Geo.Named
    {
        /// <summary>
        /// 默认构造函数
        /// </summary>
    ...
    public class NamedValueType
    {
        ...
        /// <param name="name"></param>
        /// <param name="type"></param>
        public NamedValueType(string name, ValueType type= Geo.ValueType.Str)
        {
            this.Name = name;
            this.ValueType = type;
        }
    ...
    public class TypedValue : NamedValueType
    {
        ...
        {
            Name = name;
            ValueType = Geo.Utils.EnumUtil.Parse<ValueType>(type);
